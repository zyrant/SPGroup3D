...
import numpy as np
import os
from load_scannet_data_superpoint import export
...
def export_one_scan(scan_name,
    ...
    mesh_vertices, mesh_superpoint, semantic_labels, instance_labels, unaligned_bboxes, \
    ...
    if not test_mode:
        ...
        mesh_superpoint = mesh_superpoint[mask]
    ...
    if max_num_point is not None:
        ...
        if N > max_num_point:
            ...
            mesh_superpoint = mesh_superpoint[choices]
            if not test_mode:
                semantic_labels = semantic_labels[choices]
                instance_labels = instance_labels[choices]

    np.save(f'{output_filename_prefix}_vert.npy', mesh_vertices)
    np.save(f'{output_filename_prefix}_superpoint.npy', mesh_superpoint)
    if not test_mode:
...
def main():
    ...
    parser.add_argument(
        '--output_folder',
        default='/opt/data/private/all_data/scannet_v2_0.005_10/scannet_instance_data',
        help='output folder of the result.')
    parser.add_argument(
        '--train_scannet_dir', default='/opt/data/private/all_data/scannet_v2/scans', help='scannet data directory.')
    parser.add_argument(
        '--test_scannet_dir',
        default='/opt/data/private/all_data/scannet_v2/scans_test',
        help='scannet data directory.')
    parser.add_argument(
        '--label_map_file',
        default='/opt/data/private/all_data/scannet_v2/meta_data/scannetv2-labels.combined.tsv',
        help='The path of label map file.')
    parser.add_argument(
        '--train_scan_names_file',
        default='/opt/data/private/all_data/scannet_v2/meta_data/scannet_train.txt',
        help='The path of the file that stores the scan names.')
    parser.add_argument(
        '--test_scan_names_file',
        default='/opt/data/private/all_data/scannet_v2/meta_data/scannetv2_test.txt',
        help='The path of the file that stores the scan names.')
