...
def eval_det_cls(pred, gt, id, iou_thr=None):
    ...
    for img_id in pred.keys():
        ...
        if len(gt_cur) > 0:
            ...
            ###################Debug 测试到底xyzwlhr是谁差############################
            # 换xyz不涨点，换wlh也不涨点，换xyzwlh涨点
            # ids = torch.max(iou_cur, dim=0)[1]
            # new_pre_tensor = pred_cur.tensor
            # new_pre_tensor_ids = new_pre_tensor[ids]
            # new_pre_tensor_ids[:, :6] = gt_cur.tensor[:, :6]
            # new_pre_tensor[ids] = new_pre_tensor_ids
            # new_pre = box.new_box(new_pre_tensor)
            # iou_cur = new_pre.overlaps(new_pre, gt_cur)
            #######################################################################
    ...
    # go down dets and mark TPs and FPs # 低分会被判为fp, 分就低了
    nd = len(image_ids)
    ...
    for d in range(nd):
        ...
        BBGT = R['bbox']
        cur_iou = ious[d]
        pred_label = -1  # 假设为-1，表示没有找到匹配的预测
        ...
        if len(BBGT) > 0:
            ...
            for j in range(len(BBGT)):
                ...
                if iou > iou_max:
                    iou_max = iou
                    jmax = j
                    pred_label = id  # 获取预测标签

        for iou_idx, thresh in enumerate(iou_thr):
            if iou_max > thresh:
                if not R['det'][iou_idx][jmax]:
                    true_label = id # 从gt中获取真实标签
    ...
    return ret, true_labels, pred_labels

from sklearn.metrics import confusion_matrix, classification_report

def eval_map_recall(pred, gt, label2cat, ovthresh=None):
    ...
    all_true_labels = []  # 存储所有真实类别标签
    all_pred_labels = []  # 存储所有预测类别标签

    ret_values = {}
    for classname in gt.keys():
        if classname in pred:
            ret_values[classname], true_labels, pred_labels = eval_det_cls(pred[classname],
                                                 gt[classname], classname, ovthresh)
    ...
    # 计算每个类别的精度和召回率
    class_report = classification_report(all_true_labels, all_pred_labels, target_names=label2cat.values())
    ...
    return recall, precision, ap, conf_matrix, class_report


def my_indoor_eval(gt_annos,
                dt_annos,
                metric,
                label2cat,
                logger=None,
    ...
    rec, prec, ap, conf_matrix, class_report = eval_map_recall(pred, gt, label2cat, metric)
    ret_dict = dict()
    header = ['classes']
